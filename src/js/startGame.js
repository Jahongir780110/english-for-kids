...
import excellentImage from '../img/excellent.png'
...
export const initGame = data => {
  ...
  const finishGame = () => {
    ...
    if (!errors) {
      img.setAttribute('src', excellentImage)
      img.setAttribute('alt', 'excellent')

      playAudio(successAudio)
    } else {
      img.setAttribute('src', failImage)
  ...
  cardList.querySelectorAll('.cards-list__card').forEach(cardElement => {
    cardElement.addEventListener('click', () => {
      ...
      playAudio(isCorrect ? correctAudio : errorAudio)
